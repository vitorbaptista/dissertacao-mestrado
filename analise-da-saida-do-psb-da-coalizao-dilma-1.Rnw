...
votes <- read.csv("54.csv", header = TRUE, check.names = FALSE)
votes_metadata <- read.csv("54-votacoes.csv", header = TRUE, check.names = FALSE)
@
...
mudancas_de_partidos <- read.csv("mudancas-de-partido-54-legislatura.csv")
clean_coords <- merge(clean_coords, mudancas_de_partidos[, c("id", "numero_de_partidos")], by = "id")
...
\Sexpr{formata_data(votacao_divisoria$data)} (identificador
