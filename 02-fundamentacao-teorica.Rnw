...
Considere um exemplo onde \Sexpr{nrow(parlamentares)} parlamentares votam em um
projetos de Lei que propõe a redução da maioridade penal de
\Sexpr{max(maioridade_penal)} para \Sexpr{min(maioridade_penal)} anos. Suponha
...
<<modelo-espacial-votacao, fig.height = 3, fig.cap="Preferências de 5 deputados na votação sobre a redução da maioridade penal de 18 para 16 anos.  Parlamentares a esquerda da linha de corte nos 17 anos votam sim, e os a direita votam não. O deputado com preferência sobre a linha de corte é indiferente as duas opções. Neste exemplo, é ele quem desempata a votação.">>=
