...
\begin{alltt}
\hlstd{parlamentares} \hlkwb{=} \hlkwd{data.frame}\hlstd{(}\hlkwc{maioridade_penal} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{16}\hlstd{,} \hlnum{17}\hlstd{,} \hlnum{15}\hlstd{,} \hlnum{18}\hlstd{,} \hlnum{20}\hlstd{),}
                           \hlkwc{salario_minimo} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{1000}\hlstd{,} \hlnum{900}\hlstd{,} \hlnum{1200}\hlstd{,} \hlnum{1100}\hlstd{,} \hlnum{1500}\hlstd{))}
\hlstd{maioridade_penal} \hlkwb{=} \hlkwd{c}\hlstd{(}\hlnum{16}\hlstd{,} \hlnum{18}\hlstd{)}
\hlstd{salario_minimo} \hlkwb{=} \hlkwd{c}\hlstd{(}\hlnum{1200}\hlstd{,} \hlnum{1000}\hlstd{)}
\hlstd{cutlines} \hlkwb{=} \hlkwd{data.frame}\hlstd{(}\hlkwc{maioridade_penal} \hlstd{=} \hlkwd{mean}\hlstd{(maioridade_penal),}
                      \hlkwc{salario_minimo} \hlstd{=} \hlkwd{mean}\hlstd{(salario_minimo))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{ggplot}\hlstd{(parlamentares,} \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= maioridade_penal,} \hlkwc{y} \hlstd{= salario_minimo))} \hlopt{+}
  \hlkwd{geom_vline}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{xintercept} \hlstd{= cutlines}\hlopt{$}\hlstd{maioridade_penal))} \hlopt{+}
  \hlkwd{geom_hline}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{yintercept} \hlstd{= cutlines}\hlopt{$}\hlstd{salario_minimo))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{(}\hlkwc{size} \hlstd{=} \hlnum{3}\hlstd{)} \hlopt{+}
  \hlkwd{theme_bw}\hlstd{()} \hlopt{+}
  \hlkwd{labs}\hlstd{(}\hlkwc{x} \hlstd{=} \hlstr{"Maioridade penal"}\hlstd{,} \hlkwc{y} \hlstd{=} \hlstr{"Salário mínimo"}\hlstd{)}
\end{alltt}
