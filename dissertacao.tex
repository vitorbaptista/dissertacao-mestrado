...
até 2015, entre o primeiro mandato de Fernando Henrique Cardoso até o final do
primeiro mandato de Dilma Rousseff. Excluí as 48\textordfeminine{} e
49\textordfeminine{} legislaturas, iniciadas em 1987 e 1991 respectivamente,
pois segundo \citeonline{Freitas2008} neste período os parlamentares ainda
estão se acomodando as novas regras definidas pela Constituição de 1998.
...
comportamento do parlamentar \cite{Figueiredo2001,Santos2003}, trabalhar com os
dados desagregados nos permite perceber movimentações como quando um
parlamentar deixa um partido governista para entrar em um de oposição (ou
vice-versa).

% Universo de votos

O método W-NOMINATE usado neste trabalho só interpreta votos a favor ou contra,
mas os deputados podem votar sim, não, nulo, branco, se abster ou
obstruir \cite{Carneiro2013}. Isso nos dá duas opções: ou mapeamos os outros
tipos de votos para votos a favor ou contra (por exemplo, considerando
abstenções como votos contrários), ou os ignoramos. Para evitar possíveis
problemas metodológicos ao criar esse tipo de critério, desconsidero
votos diferentes de sim ou não.
...
\gls{CEBRAP} onde mudanças de nome são identificadas pela primeira e última
sigla (e.x.: o PJ que se tornou PRN que se tornou PJC é renomeado para PJ>PTC)
e fusões são identificadas pelo nome do maior partido na data da fusão e o novo
nome (e.x.: PL e PRONA se uniram formando o PR, então temos PL>PR)
...
deputados federais na coalizão dentro de uma mesma legislatura. Por exemplo, o
deputado Arlindo Chinaglia (PT/SP) é filiado ao PT desde sua fundação em 1980 e
mantém seu cargo na Câmara desde 1995. Apesar dele passar de oposição a governo
...
seu partido se tornou oposição em 2013 é considerada, pois ocorreu durante uma
mesma legislatura.

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}
\includegraphics[width=\maxwidth]{figure/tmp/mudancas-mensais-de-coalizao-1} \caption[Número de deputados federais que mudaram de posicionamento, entrando ou saindo da coalizão entre a 50\textordfeminine{} e 54\textordfeminine{} legislaturas]{Número de deputados federais que mudaram de posicionamento, entrando ou saindo da coalizão entre a 50\textordfeminine{} e 54\textordfeminine{} legislaturas.}\label{fig:mudancas-mensais-de-coalizao}
\end{figure}
...
890 entradas e saídas da coalizão durante o período
de estudo agrupados por mês. Há meses que concentram as mudanças de
posicionamento, possivelmente um reflexo da sazonalidade das trocas de partido
\cite{Araujo2000,Melo2004,Freitas2008}. Marcamos também a data da Resolução
n\textordmasculine{} 22.610 do TSE, que em
...
coalizão, o que pode influenciar na acurácia do nosso modelo.

\section{Estimando os pontos ideais}

Para estimar a mudança de comportamento precisamos definir os períodos
analisados. Por exemplo, se quisermos analisar a influência da saída do PSB da
coalizão em outubro de 2013 no comportamento dos parlamentares, precisamos
definir quais são os períodos ``antes'' e ``depois'' da data onde supomos ter
havido uma mudança de comportamento. Períodos muito curtos, por
exemplo uma semana antes e uma semana depois, podem sofrer com a falta de
votações suficientes ou serem demasiadamente influenciados por votações
anômalas. Já períodos períodos muito longos, como 5 anos antes e 5 anos
depois, dificultam a interpretação dos resultados pois podem conter diversas
mudanças de comportamento. No mínimo, precisamos de um período grande o
suficiente para que os parlamentares participem de 20 votações com maioria de
no máximo 97,5\% dos votos\footnote{Estes são os critérios padrão de inclusão
de parlamentares e votações do W-NOMINATE, que seguimos neste trabalho.}.

Baseado nisso, defini os períodos como tendo 12 meses dentro de uma mesma
legislatura divididos em duas partes de mesmo tamanho. Eles iniciam no primeiro
dia de cada mês, desde o 01/Fev do primeiro ano da legislatura até o do
penúltimo. Por exemplo, na 54\textordfeminine{} legislatura, o primeiro período
foi de 01/Fev/2011 até 01/Fev/2012 (dividido em 01/Ago/2011), e o último foi de
01/Fev/2014 até 01/Fev/2015. Existem 37 períodos por legislatura, 185 nas 5
legislaturas estudadas neste trabalho.

Escolhidos os períodos, o próximo passo é estimar os pontos ideais de cada
parlamentar em cada um deles. Para isto, seguimos a metodologia de
\citeonline{Poole2005} descrita em
...
O modelo que usamos para criar o modelo preditivo é de aprendizagem
supervisionada. Ao contrário dos não-supervisionados, ele precisa ser treinado
com exemplos que possuem além das variáveis independentes (preditores), a
variável dependente (resposta). Para isto, precisamos definir o que
consideramos ser uma mudança de comportamento em razão de uma mudança de
posição (entrada ou saída da coalizão). Por exemplo, considere um parlamentar
que estava na coalizão desde o início da 54\textordfeminine{} legislatura em
2011, mas saiu dela no início de 2014. Se ao estimarmos sua mudança de
comportamento entre o primeiro e o segundo semestre de 2011, detectamos que ele
se afastou do governo, consideramos que essa mudança de comportamento foi um
indício da sua futura saída da coalizão? E se essa mudança de comportamento
tivesse ocorrido em 2007? Em outras palavras, precisamos definir o ``raio de
influência'' de uma saída ou entrada da coalizão.

Não há uma resposta correta. Alguns parlamentares podem mudar de comportamento
anos antes de deixarem (ou entrarem) na coalizão, enquanto outros podem não
mudar em momento algum. A hipótese fundamental deste trabalho é que eles mudam
de comportamento antes de mudar de posicionamento, caso contrário seria
impossível prever um a partir do outro. Formalmente, temos que se
$Data_{coaliz\tilde{a}o}$ seja a data da entrada ou saída da coalizão,
$Data_{mudan\textit{\c{c}}a}$ seja a data do início da mudança do comportamento,
$Data_{fim}$ seja a data final do período analisado, e $P$ seja a distância
entre $Data_{mudan\textit{\c{c}}a}$ e $Data_{fim}$, então $Data_{mudan\textit{\c{c}}a}
...
$\left[Data_{fim} - P, Data_{fim} + P\right]$.
...
% Criação dos preditores (variáveis independentes)
Neste momento, seguimos a metodologia de \citeonline{Poole2005} descrita
em \ref{cap:fundamentacao:comparando-pontos-ideais-no-tempo} para determinar as
mudanças de comportamento. Calculamos o W-NOMINATE para cada deputado federal
em cada um dos períodos, repetindo 10 vezes para estimar o erro dos pontos
ideais\footnote{\citeonline{Poole2005} sugere usarmos 101 repetições,
entretanto o volume de dados que estamos trabalhando inviabiliza essa
quantidade com os recursos computacionais que temos disponíveis. Por isto,
optei por um valor menor de repetições.}. Ao final, teremos uma tabela como a
\ref{table:dataset-final}, onde cada linha representa um parlamentar em um
período, e as colunas representam seus pontos ideais antes e depois do período
de estudo e as estimativas de erro.
...
\item[mudou\_coalizao] Variável dependente que indica se o parlamentar mudou de
posicionamento no período
\end{description}

% Variável dependente
A variável dependente usada indica se o parlamentar entrou ou saiu da coalizão
no período de estudo. Ela pode ser \verb|N|, caso ele tenha mantido sua
posição, ou \verb|S| caso a tenha alterado.
O modelo está sendo desenvolvido usando treinamento supervisionado, isto é, ele
aprende como detectar o que queremos a partir de exemplos. Para isto,
precisamos definir o que queremos detectar. Em outras palavras, vamos definir
quando uma mudança de comportamento indica uma futura mudança de posicionamento
ou não.
...
ponto ideal por causa de nossos critérios, ficamos com 47.851
entradas na tabela. Dessas, 9,73\%
(4.655) são de deputados que mudaram
de posicionamento e 90,27\%
(43.196) que não. A tabela
...
\begin{itemize}\raggedright
  ...
  \item Other packages: caret~6.0-47, doParallel~1.0.8,
    ...
    knitr~1.9, lattice~0.20-31, randomForest~4.6-10
  \item Loaded via a namespace (and not attached):
    ...
    Rcpp~0.11.4, reshape2~1.4.1, scales~0.2.4, SparseM~1.6,
