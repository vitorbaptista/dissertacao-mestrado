...
<<>>=
trainIndex = which(data$legislature != 54)
train = data[ trainIndex,]
test = data[ -trainIndex,]

train_proportions = table(train$changed_coalition)
test_proportions = table(test$changed_coalition)
@
...
uma suposição improvável. O que nos interessa medir é a performance do modelo
em dados atuais. Por isto, separei os dados da 54\textordfeminine{} legislatura
para teste e restante para treinamento. O conjunto de treino possui
\Sexpr{nrow(train)} (\Sexpr{format_percent(nrow(train) / nrow(data))}) linhas
com \Sexpr{format_percent(train_proportions["S"] / nrow(train))} sendo de
mudanças de posicionamento, enquanto o de teste possui \Sexpr{nrow(test)}
(\Sexpr{format_percent(nrow(test) / nrow(data))}) linhas com
\Sexpr{format_percent(test_proportions["S"] / nrow(test))} sendo de mudanças de
posicionamento.


% Vou usar duas divisões:
