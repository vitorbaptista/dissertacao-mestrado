...
\Sexpr{data_resolucao_infidelidade} alterou o entendimento das
...
<<>>=
tmp.data_proportions = table(data$changed_coalition) / nrow(data)
@
...
visto anteriormente, das \Sexpr{nrow(data)} observações,
\Sexpr{format_percent(tmp.data_proportions["S"])}
(\Sexpr{length(which(data$changed_coalition == "S"))}) são relativas a mudanças
de posicionamento, e \Sexpr{format_percent(tmp.data_proportions["N"])}
(\Sexpr{length(which(data$changed_coalition == "N"))}) não.
...
da performance em 60\% dos dados da 54\textordfeminine{} legislatura, chamadado
...
parâmetros, nos conjuntos de treino e validação juntos;
\item Define o ponto de corte para classificação usando metade dos dados da
54\textordfeminine{} legislatura não usados para validação;
...
\Sexpr{format_percent(testing_roc_proportions["S"])}
...
mantidas o mais próximo possível.
...
treino, foram usadas \Sexpr{format_percent(nrow(training.downsampled) /
nrow(training.original))} (\Sexpr{nrow(training.downsampled)}), sendo
...
sendo, respectivamente, \Sexpr{format_percent(validation_proportions["S"])} e
\Sexpr{format_percent(testing_proportions["S"])} de mudanças de posicionamento.
...
\Sexpr{ci(rocs.validation$C5.0)[[2]]} (\Sexpr{ci(rocs.validation$C5.0)[[1]]},
\Sexpr{ci(rocs.validation$C5.0)[[3]]}), contra
\Sexpr{ci(rocs.validation$nnet)[[2]]} (\Sexpr{ci(rocs.validation$nnet)[[1]]},
\Sexpr{ci(rocs.validation$nnet)[[3]]}) do segundo lugar, que usou redes
...
\begin{tabular}{l l c c c c}
  ...
  \hline
  C5.0 & model & \Sexpr{models$C5.0$bestTune$model} & \Sexpr{paste(lapply(ci(rocs.validation$C5.0), format_inline), collapse = "&")} \\
       & winnow & \Sexpr{models$C5.0$bestTune$winnow} & & & \\
       & trials & \Sexpr{models$C5.0$bestTune$trials} & & & \\
  NNET & size & \Sexpr{models$nnet$bestTune$size} & \Sexpr{paste(lapply(ci(rocs.validation$nnet), format_inline), collapse = "&")} \\
       & decay & \Sexpr{format_number(models$nnet$bestTune$decay)} & & & \\
  GBM & shrinkage & \Sexpr{models$gbm$bestTune$shrinkage} & \Sexpr{paste(lapply(ci(rocs.validation$gbm), format_inline), collapse = "&")} \\
      & interaction.depth & \Sexpr{models$gbm$bestTune$interaction.depth} & & & \\
      & n.minobsinnode & \Sexpr{models$gbm$bestTune$n.minobsinnode} & & & \\
      & n.trees & \Sexpr{models$gbm$bestTune$n.trees} & & & \\
  RF & mtry & \Sexpr{models$rf$bestTune$mtry} & \Sexpr{paste(lapply(ci(rocs.validation$rf), format_inline), collapse = "&")} \\
  SVM Radial & C & \Sexpr{models$svmRadial$bestTune$C} & \Sexpr{paste(lapply(ci(rocs.validation$svmRadial), format_inline), collapse = "&")} \\
             & sigma & \Sexpr{gsub("\\.", ",", models$svmRadial$bestTune$sigma)} & & & \\
...
<<find-cutoff, fig.cap="Curva ROC do modelo final com o ponto de corte escolhido em destaque.">>=
@

Definidos o modelo e seus parâmetros, treinaremos o modelo final com o conjunto
de treino e validação juntos. Como o conjunto de validação não é balanceado,
tendo \Sexpr{format_percent(validation_proportions["S"])}
(\Sexpr{length(which(validation$changed_coalition == "S"))}) mudanças de
posicionamento, ele precisa ser \emph{downsampled}. Ao final, unindo o conjunto
de treino (que já foi \emph{downsampled}) com o de validação, o novo conjunto
possui \Sexpr{nrow(training.final)} observações, sendo
...
\ref{cap:fundamentacao-teorica:avaliando-modelos-de-classificacao}, a maioria
das ferramentas de aprendizagem de máquina definem como ponto de corte padrão
0,5. Apesar disso, especialmente em casos como o nosso em que os dados não são
balanceados, é útil analisar outros pontos de corte buscando um melhor (que
depende de cada caso). Para este modelo, meu objetivo é minimizar o número de
falsos positivos (parlamentares que não mudaram de posicionamento sendo
classificados como se tivessem mudado). Para entender a razão, considere o
seguinte exemplo:
...
A figura \ref{fig:find-cutoff} mostra a curva ROC do conjunto de dados de
definição do ponto de corte em dois pontos: 0,5 e \Sexpr{cutoffPoint}.
Considerando o corte em 0,5, no exemplo acima \Sexpr{tmp.tp_05 + tmp.fp_05}
parlamentares seriam classificados como tendo mudado de posicionamento,
sendo \Sexpr{format_percent(tmp.fp_05 / (tmp.tp_05 + tmp.fp_05))}
(\Sexpr{tmp.fp_05}) falsos positivos e \Sexpr{tmp.fn_05} falsos negativos. Já
com o corte em \Sexpr{cutoffPoint}, \Sexpr{tmp.tp_best + tmp.fp_best} seriam
classificados como tendo
mudado de posicionamento, sendo \Sexpr{format_percent(tmp.fp_best /
(tmp.tp_best + tmp.fp_best))} (\Sexpr{tmp.fp_best}) falsos positivos
e \Sexpr{tmp.fn_best} falsos negativos. Em outras palavras, ao mudar o ponto de
corte de 0,5 para \Sexpr{cutoffPoint}, estamos trocando \Sexpr{tmp.fp_05 -
tmp.fp_best} falsos positivos a menos por \Sexpr{tmp.fn_best - tmp.fn_05}
falsos negativos a mais.

<<bootstrap-roc-and-confusion-matrix-final-model>>=
@

A área sob a curva ROC (figura \ref{fig:roc-final-model}) alcançada pelo modelo
final foi de \Sexpr{ci(rocs.testing$final)[[2]]}
(\Sexpr{ci(rocs.testing$final)[[1]]}, \Sexpr{ci(rocs.testing$final)[[3]]}), com
Kappa \Sexpr{confMatrix$overall[["Kappa"]]} e
\Sexpr{format_percent(confMatrix$byClass[["Sensitivity"]])} das
mudanças (sensibilidade) e
\Sexpr{format_percent(confMatrix$byClass[["Specificity"]])} das
não-mudanças de posicionamento (especificidade) classificadas corretamente
(considerando o ponto de corte em \Sexpr{cutoffPoint}). De acordo com a
classificação de \citeonline{HosmerJr2013} descrita na tabela
\ref{table:valores-auc}, o modelo possui um poder de discriminação excelente.
...
% \Sexpr{rocs.validation$glm$auc} no conjunto de validação.
%
% \subsubsection*{SVM Linear}
%
% Parametro ``C'' (custo) mantido constante em 1. Área sob a curva ROC
% \Sexpr{rocs.validation$svmLinear$auc} no conjunto de validação.
%
% \subsubsection*{SVM Radial}
%
% Parametros sigma mantido constante em
% \Sexpr{unique(models$svmRadial$results$sigma)} e custo variando em
% \Sexpr{sort(unique(models$svmRadial$results$C))}. Área sob a curva ROC
% \Sexpr{rocs.validation$svmRadial$auc} no conjunto de validação.
%
% \subsubsection*{GBM}
%
% Parametros shrinkage constante em \Sexpr{unique(models$gbm$results$shrinkage)},
% interaction.depth em \Sexpr{unique(models$gbm$results$interaction.depth)},
% n.minobsinnode em \Sexpr{unique(models$gbm$results$n.minobsinnode)}, e n.trees
% em \Sexpr{unique(models$gbm$results$n.trees)}. Área sob a curva ROC
% \Sexpr{rocs.validation$gbm$auc} no conjunto de validação.
%
% \subsubsection*{RF}
%
% Parâmetro mtry em \Sexpr{unique(models$rf$results$mtry)}. Área sob a curva ROC
% \Sexpr{rocs.validation$rf$auc} no conjunto de validação.
%
...
% \subsubsection*{C5.0}
%
% Parâmetros model em \Sexpr{unique(models$C5.0$results$model)}, winnow em
% \Sexpr{unique(models$C5.0$results$winnow)} e trials em
% \Sexpr{unique(models$C5.0$results$trials)}. Área sob a curva ROC
% \Sexpr{rocs.validation$C5.0$auc} no conjunto de validação.
%
% \subsubsection*{NNET}
%
% Parâmetros size em \Sexpr{unique(models$nnet$results$size)} e decay em
% \Sexpr{unique(models$nnet$results$decay)}. Área sob a curva ROC
% \Sexpr{rocs.validation$nnet$auc} no conjunto de validação.
