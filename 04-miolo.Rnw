...
<<raio-de-influencia-coalizao, dev='tikz', fig.height=1, fig.cap='Diversos períodos de análise de um parlamentar que mudou de comportamento em $Data_{coalizao} - P$, representado pela mudança da região branca para a verde.', fig.subcap=c('A data final está no limite do raio de influência da coalizão.', 'O raio de influência da coalizão chega a metade do segundo período. A partir daqui começamos a considerar que houve uma mudança de posicionamento.', 'A data estimada de mudança se iguala a data real. Este é o período onde esperamos encontrar a maior mudança de comportamento.', 'A partir daqui, o comportamento começa a se igualar nos dois momentos de análise. Este é o último período em que consideramos que houve uma mudança de posicionamento.', 'O comportamento nos dois períodos se iguala. Esperamos que a diferença entre os pontos ideias dos dois momentos volte aos níveis da primeira figura.')>>=
plot_data = function(data) {
  p = ggplot(data, aes(x = x, y = c(0))) +
        ...
        labs(x = "", y = "") +
        geom_rect(xmin = 12, xmax = Inf, ymin = -Inf, ymax = Inf, alpha = 0.1, fill = "green") +
        geom_rect(xmin = min(data$x), xmax = max(data[1:3,]$x), ymin = -Inf, ymax = Inf, alpha = 0.1, fill = "grey") +
...
data_mudanca = "$Data_{mudan\\textit{\\c{c}}a}$"
data_final = "$Data_{final}$"
data_coalizao = "$Data_{coaliz\\tilde{a}o}$"

t0 = data.frame(x = c(0, 6, 12, 18),
                label = c(data_inicial, data_mudanca, data_final, data_coalizao),
                linetype = c(rep("solid", 3), "dashed"))
t1 = data.frame(x = c(3, 9, 15, 18),
                label = c(data_inicial, data_mudanca, data_final, data_coalizao),
                linetype = c(rep("solid", 3), "dashed"))
t2 = data.frame(x = c(6, 12, 18),
                label = c(data_inicial, data_mudanca, paste(data_final, "$=$", data_coalizao)),
                linetype = rep("solid", 3))
t3 = data.frame(x = c(9, 15, 21, 18),
                label = c(data_inicial, data_mudanca, data_final, data_coalizao),
                linetype = c(rep("solid", 3), "dashed"))
t4 = data.frame(x = c(12, 18, 24),
                label = c(data_inicial, paste(data_mudanca, "$=$", data_coalizao), data_final),
                linetype = rep("solid", 3))
