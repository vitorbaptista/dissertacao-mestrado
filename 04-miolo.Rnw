...
\Sexpr{nrow(coalition_changes)} entradas e saídas da coalizão durante o período
...
\begin{figure}
<<fig.height = 2>>=
data = data.frame(x = c(0, 6, 12),
                  label = c("Data inicial", "Data de mudança", "Data final"))
workaround = data.frame(x = c(-1, 15),
                        label = c("", ""))

ggplot(rbind(data, workaround), aes(x = x, y = c(0))) +
  ...
  labs(x = "", y = "") +
  geom_rect(xmin = 9, xmax = 15, ymin = -Inf, ymax = Inf, alpha = 0.1, fill = "green") +
  geom_vline(xintercept = data$x) +
  scale_x_discrete(breaks = data$x, labels=data$label)
@
\end{figure}
