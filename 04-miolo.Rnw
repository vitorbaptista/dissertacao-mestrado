...
tmp = data.frame(ponto_ideal = c(training.original$before, training.original$after,
                 ...
                 Dados = c(rep("Original", nrow(training.original) * 2),
                           rep("Downsampled", nrow(training) * 2)))
tmp$dataset = factor(tmp$Dados, levels = c("Original", "Downsampled"))
ggplot(tmp, aes(x = ponto_ideal)) +
  theme(legend.position = "right") +
  geom_density(aes(fill = Dados, colour = Dados), alpha = 0.3) +
  labs(x = "Ponto ideal", y = "Densidade")
@
...
<<bootstrap-roc-and-confusion-matrix-final-model, include=FALSE>>=
@
...
\Sexpr{ci(rocs.testing$final)[[2]]} (\Sexpr{ci(rocs.testing$final)[[1]]},
\Sexpr{ci(rocs.testing$final)[[3]]}), com Kappa
\Sexpr{confusionMatrix.testing$overall[["Kappa"]]} e
\Sexpr{format_percent(confusionMatrix.testing$byClass[["Specificity"]])} das
mudanças (especificidade) e
\Sexpr{format_percent(confusionMatrix.testing$byClass[["Sensitivity"]])}
das não-mudanças de posicionamento (sensibilidade) classificadas corretamente
(considerando o ponto de corte em \Sexpr{cutoffPoint}).
...
teste, com o ponto de corte marcado em \Sexpr{cutoffPoint}.

<<roc-final-model, fig.cap="Curva ROC do modelo final nos dados de teste.">>=
plot(rocs.testing$final,
...
<<>>=
kable(confusionMatrix.testing$table)
@

<<echo=TRUE>>=
confusionMatrix.testing
attributes(confusionMatrix.testing)
confusionMatrix.testing$overall
confusionMatrix.testing$byClass
@
