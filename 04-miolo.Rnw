...
diferentes, escolhendo o final de acordo com a \glsfirst{AUC}.
...
\citeonline{Poole2005} usando o modelo W-NOMINATE \cite{Poole1985} (ver Secão
...
(\Sexpr{nrow(data_null)}) períodos, pois ou parlamentar não participou de
...
casos foram excluídos, restando ao final \Sexpr{nrow(data)} observações.
...
(\Sexpr{sum(migrators_per_legislature_year[c(1, 3)])}), está concentrada no
...
\Sexpr{format_percent(data_proportions["S"])} (\Sexpr{data_proportions["S"] *
nrow(data)}) são relativas a mudanças de posicionamento, e
\Sexpr{format_percent(data_proportions["N"])} (\Sexpr{data_proportions["N"] *
nrow(data)}) não.
...
<<variaveis-independentes, fig.cap="Densidade das variáveis independentes dos parlamentares em períodos que mudaram ou não de posicionamento.">>=
...
recentes, seguimos a seguinte estratégia \cite{Kuhn2013}:

<<dataset-split-graphs, fig.cap="Divisões do conjunto de dados usadas.", fig.subcap=c("Divisão do conjunto de dados usada para definir o tipo do modelo final e seus parâmetros.", "Divisão do conjunto de dados usado para treinar o modelo final, definir seu o ponto de corte e estimar sua performance.")>>=
...
feitas. Seguindo essa estratégia, os \Sexpr{nrow(data)} dados serão separados em
...
\Sexpr{format_percent(data_proportions["N"])} de acurácia simplesmente
...
\Sexpr{format_confusion_matrix(confMatrix.roc_05)}
\end{subtable}%
\begin{subtable}{.5\linewidth}
\centering
\caption{Matriz de confusão com ponto de corte \Sexpr{cutoffPoint}.}
\Sexpr{format_confusion_matrix(confMatrix.roc_best)}
\end{subtable}
\end{table}

<<stats-model-on-testing-cutoff-dataset>>=
tmp.pos_05 = sum(confMatrix.roc_05$table[1,])
tmp.tp_05 = confMatrix.roc_05$table[[1, 1]]
tmp.neg_05 = sum(confMatrix.roc_05$table[2,])
tmp.tn_05 = confMatrix.roc_05$table[[2, 2]]

tmp.pos_best = sum(confMatrix.roc_best$table[1,])
tmp.tp_best = confMatrix.roc_best$table[[1, 1]]
tmp.neg_best = sum(confMatrix.roc_best$table[2,])
tmp.tn_best = confMatrix.roc_best$table[[2, 2]]
@
...
0,5 e o escolhido \Sexpr{cutoffPoint}. Considerando o corte 0,5,
\Sexpr{tmp.pos_05} parlamentares seriam classificados como tendo mudado de
posicionamento \Sexpr{format_percent(tmp.tp_05 / tmp.pos_05)}
(\Sexpr{tmp.tp_05}) verdadeiros positivos e \Sexpr{tmp.neg_05} seriam
classificados como não tendo mudado de posicionamento, sendo
\Sexpr{format_percent(tmp.tn_05 / tmp.neg_05)} (\Sexpr{tmp.tn_05}) verdadeiros
negativos. Já com o corte em \Sexpr{cutoffPoint}, \Sexpr{tmp.pos_best}
parlamentares seriam classificados como tendo mudado de posicionamento, sendo
\Sexpr{format_percent(tmp.tp_best / tmp.pos_best)} (\Sexpr{tmp.tp_best})
verdadeiros positivos e \Sexpr{tmp.neg_best} seriam classificados como não
tendo mudado de posicionamento, sendo \Sexpr{format_percent(tmp.tn_best /
tmp.neg_best)} (\Sexpr{tmp.tn_best}) verdadeiros negativos. Em outras palavras,
ao mudar o ponto de corte de 0,5 para \Sexpr{cutoffPoint}, estamos trocando
\Sexpr{tmp.tp_05 - tmp.tp_best} verdadeiros positivos a menos por
\Sexpr{tmp.tn_best - tmp.tn_05} verdadeiros negativos a mais. A Tabela
...
corte em \Sexpr{cutoffPoint}, o modelo possui Kappa
...
\centering
\Sexpr{format_confusion_matrix(confMatrix)}
\end{table}
...
\glsfirst{AUC}, o modelo que usou C5.0 foi escolhido. Após a definição do ponto
de corte, ele alcançou um \gls{AUC} \Sexpr{ci(rocs.testing$final)[[2]]} no
conjunto de teste, tendo um poder de discriminação excelente de acordo com a
classificação de \citeonline{HosmerJr2013}.
