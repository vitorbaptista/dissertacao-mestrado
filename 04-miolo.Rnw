...
<<>>=
coalition_changes$rollcall_month = as.numeric(format(coalition_changes$rollcall_date, "%m"))
coalition_changes_per_month = table(coalition_changes$rollcall_month)
coalition_changes_per_month_percent = coalition_changes_per_month / nrow(coalition_changes)
coalition_changes$migrated = as.character(coalition_changes$party_before) != as.character(coalition_changes$party_after)
...
outubro, concentram
\Sexpr{format_percent(sum(coalition_changes_per_month_percent[c(3, 10)]))}
(\Sexpr{sum(coalition_changes_per_month[c(3, 10)])}) das mudanças, o que pode
...
ggplot(coalition_changes, aes(x = as.Date(paste0("2014-", coalition_changes$rollcall_month, "-01")))) +
  geom_histogram(binwidth = 10) +
  scale_x_date(labels = date_format("%b")) +
  labs(x = "Mês", y = "Mudanças de posicionamento")
@
...
período, \Sexpr{format_percent(length(which(coalition_changes$migrated)) /
...
comportamento \Sexpr{median(migrators_per_month)} vezes por
mês em mediana com \Sexpr{format_percent(migrators_per_month_percent[10])}
(\Sexpr{migrators_per_month[10]}) ocorrendo em outubro. Houve
respectivamente \Sexpr{migrators_per_month[3]} e \Sexpr{migrators_per_month[2]} em
...
posicionamento \Sexpr{mean(non_migrators_per_month)} vezes por mês em mediana
...
<<mudancas-de-posicionamento-por-migrantes, fig.cap="Número de deputados que mudaram de posicionamento entre a 50\\textordfeminine{} e 54\\textordfeminine{} legislaturas agregados por mês.">>=
migrated_labeller = function (variable, value) {
  if (variable != "migrated") {
    stop("The 'migrated_labeller' function only works for the 'migrated' variable")
  }
  list("Não-migrante", "Migrante")
}

ggplot(coalition_changes, aes(x = as.Date(paste0("2014-", rollcall_month, "-01")))) +
