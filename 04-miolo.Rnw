...
<<the-label, dev='tikz', fig.cap='Diversos períodos de análise de um parlamentar que mudou de comportamento 6 meses antes de entrar ou sair da coalizão. Na região branca ele teve um comportamento, e na região verde teve outro.', fig.height=1, fig.subcap=c('A data final está no limite do raio de influência da coalizão', 'O raio de influência da coalizão chega a metade do segundo período', 'A mudança de comportamento influencia blah', 'A mudança de comportamento está na metade do período inicial', 'O período inteiro ocorre depois da mudança de comportamento')>>=
