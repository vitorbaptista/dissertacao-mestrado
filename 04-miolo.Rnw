...
<<>>=
trainingIndex = which(data$legislature != 54)
training = data[trainingIndex,]
remaining = data[-trainingIndex,]

set.seed(1)
testingIndex = createDataPartition(paste(remaining$changed_coalition,
                                         remaining$mid_vote_month,
                                         remaining$mid_vote_legislature_year),
                                   p = .25,
                                   list = FALSE)
testing = remaining[testingIndex,]
validation = remaining[-testingIndex,]

training_proportions = table(training$changed_coalition) / nrow(training)
...
Seguindo esses passos, os \Sexpr{nrow(data)} dados serão separados em
\Sexpr{format_percent(nrow(training) / nrow(data))} (\Sexpr{nrow(training)})
para treino, \Sexpr{format_percent(nrow(validation) / nrow(data))}
(\Sexpr{nrow(validation)}) para validação e \Sexpr{format_percent(nrow(testing)
/ nrow(data))} (\Sexpr{nrow(testing)}) para teste. Eles possuem
respectivamente \Sexpr{format_percent(training_proportions["S"])},
\Sexpr{format_percent(validation_proportions["S"])} e
\Sexpr{format_percent(testing_proportions["S"])} de mudanças de
...
<<densidade-validacao-teste, fig.cap="Densidade de mudanças e não-mudanças de posicionamento por mês em cada ano da legislatura nos conjuntos de dados de validação e teste. Visualmente, a densidade dos dois conjuntos é parecida.">>=
aux = data.frame(changed_coalition = c(validation$changed_coalition,
                                       testing$changed_coalition),
                 month = c(validation$mid_vote_month,
                           testing$mid_vote_month),
                 legislature_year = c(validation$mid_vote_legislature_year,
                                      testing$mid_vote_legislature_year),
                 dataset = c(rep("Validação", nrow(validation)),
                             rep("Teste", nrow(testing))))

aux$changed_coalition = factor(aux$changed_coalition, labels = c("Sim", "Não"))
aux$month = as.Date(paste0("2014-", aux$month, "-01"))
aux$dataset = factor(aux$dataset, levels = c("Validação", "Teste"))

ggplot(aux, aes(x = month, fill = changed_coalition)) +
...
Por fim, há mais um último problema a ser resolvido. A proporção de dados de
parlamentares que mudaram de posicionamento (segundo os critérios definidos em
XXX) dos que não mudaram está muito desbalanceada. Cerca de 95\% dos dados são
de parlamentares em períodos em que não houve uma mudança de posicionamento.
Isto pode fazer com que os modelos preditivos simplesmente prevejam todos
exemplos como sendo negativos (i.e. em que não houve mudança de posicionamento)
e, assim, conseguir uma acurácia de 95\%.

Para evitar isso, alguns modelos aceitam um parâmetro para configurar o
``custo'' de classificar erradamente cada classe. Ao dizer que, por exemplo,
errar classificando um parlamentar como se ele não tivesse mudado de
posicionamento quando, na realidade, ele mudou, é 10 vezes mais custoso do que
classificar um parlamentar que não mudou de posicionamento como se tivesse
mudado.

Como nem todo modelo aceita esses parâmetros, isso limitaria nossa
escolha. Por isso, preferi usar a técnica \emph{downsampling}, que diminui o
número de exemplos da maior classe até chegar próximo de uma divisão uniforme,
com 50\% dos exemplos pertencendo a cada classe. Ao fazer o
\emph{downsampling}, é preciso se certificar que a única característica
diferente dele para o conjunto completo é a proporção de exemplos de cada
classe, sendo semelhantes nas outras características. Neste trabalho, garanti
que a proporção de exemplos por legislatura, mês e ano da legislatura sejam
similares.
